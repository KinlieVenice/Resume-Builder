...
const { listJobs, getJob, findJobByLink, createJob, updateJob, deleteJob, parseJobFields } = require('./jobsStore');
...
test('findJobByLink finds an existing job for that person by exact link match', () => {
  const db = testDb();
  const job = createJob(db, {
    personId: 'ada',
    dateApplied: '2026-09-19',
  ...
  const found = findJobByLink(db, 'ada', 'https://example.com/job/1');
  assert.equal(found.id, job.id);
});

test('findJobByLink returns null when no match, when link is empty, or for a different person', () => {
  const db = testDb();
  createJob(db, { personId: 'ada', dateApplied: '2026-09-19', status: 'Submitted', link: 'https://example.com/job/1' });

  assert.equal(findJobByLink(db, 'ada', 'https://example.com/job/999'), null);
  assert.equal(findJobByLink(db, 'ada', ''), null);
  assert.equal(findJobByLink(db, 'alan', 'https://example.com/job/1'), null);
});
